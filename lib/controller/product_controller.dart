...
class ProductController extends GetxController {
  ...
  void fetchData() async {
    ...
    try {
      final data = await remoteState.api
          .respone("${AppLinks.product}?op_type=select&family_id=$id");
      if (data != null) {
  ...
  void detailsViewScreen(
      {int? id, String? name, int? price, List<String>? images}) async {
    Get.to(() => const DetailsView2(),
        binding: DetailsViewBinding2(),
        duration: const Duration(milliseconds: 400),
        curve: Curves.ease,
        transition: Transition.leftToRightWithFade,
        arguments: {"id": id, "name": name, "price": price, "Images": images});
  }
}

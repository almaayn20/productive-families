...
class DetailsViewController2 extends GetxController {
  ...
  void onInit() {
    super.onInit();
    var temp = Get.arguments["Images"];
    id = Get.arguments["id"];
    name = Get.arguments["name"];
    price = Get.arguments["price"];
    images.value = temp == null ? [] : temp as List<String>;
    fetchData();
  ...
  void fetchData() async {
    ...
    log(id.toString());
    try {
      var response = await remoteState.api
          .respone("${AppLinks.photo}?op_type=select&product_id=$id");
      if (response != null) {
